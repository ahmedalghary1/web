...
from maintenance.models import ChecklistItem, ChecklistTemplate, FactoryMaintenanceState, MaintenanceReport, MaintenanceReportItem, EmergencyMaintenanceItem
...
class MaintenanceCycleService:
    ...
    @classmethod
    def validate_emergency_items(cls, factory, items):
        if not items: return
        asset_ids = [item["asset_id"] for item in items]
        valid_assets = set(Asset.objects.filter(factory=factory, id__in=asset_ids, is_active=True, is_archived=False).values_list("id", flat=True))
        for item in items:
            if item["asset_id"] not in valid_assets:
                raise ValidationError({"emergency_items": "الماكينة المحددة في الصيانة الطارئة غير صالحة أو غير تابعة للمصنع."})
    ...
    def upsert_report(cls, supervisor, data):
        ...
        if existing:
            ...
            cls.validate_answers(existing.asset, data["answers"])
            cls.validate_emergency_items(supervisor.factory, data.get("emergency_items", []))
            cls._replace_answers(existing, data["answers"])
            cls._replace_emergency_items(existing, data.get("emergency_items", []))
            for field in ("started_at_device", "completed_at_device", "last_modified_at_device"): setattr(existing, field, data[field])
            for field in ("cleaner_name", "mechanical_technician", "electrical_technician", "maintenance_manager"):
                if field in data:
                    setattr(existing, field, data[field])
            existing.save(update_fields=[
                "started_at_device", "completed_at_device", "last_modified_at_device",
        ...
        if due.id != asset.id: return SyncResult("conflict", reason=f"الماكينة لا تطابق الماكينة المطلوبة لهذا اليوم ({due.asset_code}). يرجى إعادة التعيين والبدء في الماكينة المحددة.")
        ...
        report = MaintenanceReport.objects.create(
            ...
            started_at_device=data["started_at_device"],
            completed_at_device=data["completed_at_device"],
            last_modified_at_device=data["last_modified_at_device"],
            cleaner_name=data.get("cleaner_name", ""),
            mechanical_technician=data.get("mechanical_technician", ""),
            electrical_technician=data.get("electrical_technician", ""),
            maintenance_manager=data.get("maintenance_manager", ""),
            is_locked=data["report_date"] < timezone.localdate()
        )
        cls._replace_answers(report, data["answers"])
        cls._replace_emergency_items(report, data.get("emergency_items", []))
    ...
    @staticmethod
    def _replace_emergency_items(report, items):
        report.emergency_items.all().delete()
        if not items: return
        EmergencyMaintenanceItem.objects.bulk_create([
            EmergencyMaintenanceItem(
                report=report,
                asset_id=item["asset_id"],
                issue_description=item["issue_description"].strip(),
