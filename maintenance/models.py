...
class FactoryMaintenanceState(models.Model):
    factory = models.OneToOneField("factories.Factory", on_delete=models.CASCADE, related_name="maintenance_state"); current_asset = models.ForeignKey("assets.Asset", null=True, blank=True, on_delete=models.PROTECT, related_name="current_for_states"); current_effective_date = models.DateField(null=True, blank=True); last_completed_asset = models.ForeignKey("assets.Asset", null=True, blank=True, on_delete=models.PROTECT, related_name="completed_for_states"); last_completed_date = models.DateField(null=True, blank=True, db_index=True); order_version = models.PositiveIntegerField(default=1); manual_selected_by = models.ForeignKey(settings.AUTH_USER_MODEL, null=True, blank=True, on_delete=models.SET_NULL, related_name="manual_maintenance_selections"); manual_selected_at = models.DateTimeField(null=True, blank=True); updated_at = models.DateTimeField(auto_now=True)
