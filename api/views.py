...
def current_payload(user):
    ...
    report = MaintenanceReport.objects.filter(factory=user.factory, report_date=today).select_related("asset", "supervisor").prefetch_related("answers__checklist_item", "emergency_items__asset").first()
