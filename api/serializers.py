...
from maintenance.models import ChecklistTemplate, MaintenanceReport, MaintenanceReportItem

class FactorySerializer(serializers.ModelSerializer):
    class Meta: model = Factory; fields = ["id", "name", "code"]
class UserSerializer(serializers.ModelSerializer):
    factory = FactorySerializer(read_only=True)
    class Meta: model = User; fields = ["id", "phone", "role", "factory"]
...
class ReportSerializer(serializers.ModelSerializer):
    answers = ReportItemSerializer(many=True, read_only=True); asset = AssetSerializer(read_only=True); is_locked = serializers.SerializerMethodField()
    class Meta: model = MaintenanceReport; fields = ["id", "client_report_id", "factory_id", "asset", "supervisor_id", "report_date", "started_at_device", "completed_at_device", "last_modified_at_device", "created_at", "updated_at", "completed_at", "is_locked", "answers"]
